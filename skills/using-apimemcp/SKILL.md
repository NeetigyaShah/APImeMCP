...
| `execute_native_extraction` | `targetUrl?`, `templateId?`, `proxyUrl?`, `cookieString?`, `connectionId?`, `executableScript?`, `outputSchema?` | Runs an extraction OR an action-sequence template. A non-empty `executableScript` makes a dry-run that bypasses template lookup and all storage persistence; templates with `outputSchema` return `schemaValidation: { valid, errors? }`, otherwise it is absent. Prefer a connected `connectionId` for login state; it uses a persistent browser profile and cannot be combined with manual cookie input. Logs a metric on success. |
